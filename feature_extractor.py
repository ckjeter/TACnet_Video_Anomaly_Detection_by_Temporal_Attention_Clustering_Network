...
def extract(device, net, input_path):
    ...
    dataloader = DataLoader(dataset, batch_size = 50, shuffle=False, num_workers=50)
    feature = []
...
def path_generate(root, video, test=False):
    if not test:
        if video.find("Normal") >= 0:
            input_path = os.path.join(root, video.split(".")[0], 'frame')
            output_path = os.path.join('features', 'train', video.split("/")[-1].split("_x")[0] + '.npy')
        else:
            input_path = os.path.join(root, "Anomaly-Videos", video.split(".")[0], 'frame')
            output_path = os.path.join('features', 'train', video.split("/")[-1].split("_")[0] + '.npy')
    else:
        if video.find("Normal") >= 0:
            input_path = os.path.join(root, video.split(".")[0], 'frame')
            output_path = os.path.join('features', 'test', video.split("/")[-1].split("_x")[0] + '.npy')
        else:
            input_path = os.path.join(root, "Anomaly-Videos", video.split(".")[0], 'frame')
            output_path = os.path.join('features', 'test', video.split("/")[-1].split("_")[0] + '.npy')
...
def main():
    ...
    for video in data_list:
        input_path, output_path = path_generate(config.root, video, test=False)
    ...
    for data in data_list_test:
        ...
        input_path, output_path = path_generate(config.root, video, test=True)
    ...
    for paths in trainlist:
        ...
        if not os.path.exists(output_path):
            feature = extract(device, backbone, input_path)
    ...
    for paths in testlist:
        ...
        if not os.path.exists(output_path):
            feature = extract(device, backbone, input_path)
            output_file = np.array((np.array(label), feature), dtype=object)
